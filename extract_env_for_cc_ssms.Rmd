...
```{r}
## daily SST (daily SST Product is -180/180)
...
sstamean <- rbindlist(lapply(sstafilesIn, FUN=getvar,varname=varname,ptsfile=obsdata,pt.radius=mnradius),fill=TRUE)
...
# varstring = c('all_vars', 'water_u', 'water_v')
    ...
    for(v in varstring){
        ...
        if(v == "ESPC-D-V02") {
            ...
            hycomfilesIn = getDateRangeX(inpts=obsdata, ncIn=hycomfiles_u) #%>% .[grepl("u3z", .)]
        } else {
        ...
        if(v == "ESPC-D-V02") {
            ...
            hycomfilesIn = getDateRangeX(inpts=obsdata, ncIn=hycomfiles_v) #%>% .[grepl("v3z", .)]
        } else {
            hycomfilesIn <- getDateRangeX(inpts=obsdata, ncIn=hycomfiles) 
        }
