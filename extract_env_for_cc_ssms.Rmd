...
if('multi-4km_P1D' %in% params$envs){
    ...
    # add to obsdata
    obsdata$globcolour_daily_new_mean<-rep(NA, length(obsdata[,1]))
    obsdata$globcolour_daily_new_mean[globcolourmean$X]<-globcolourmean$CHL_mean
    
    # preview results
    head(obsdata)
    range(obsdata$globcolour_daily_new_mean, na.rm=T)
